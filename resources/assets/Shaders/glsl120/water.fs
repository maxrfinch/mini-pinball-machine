...
void main() {
    ...
    float ripple = texture2D(rippleTex, vec2(fragTexCoord.x, 0.5)).r;
    ...
    float left = texture2D(rippleTex, vec2(fragTexCoord.x - dx, 0.5)).r;
    float right = texture2D(rippleTex, vec2(fragTexCoord.x + dx, 0.5)).r;
