...
static const LedBoard boards[NEOPIXEL_BOARDS] = {
    {1, 0, 7, false, BOARD_POS_RIGHT_REAR},    // Board 1: Right rear (1-8 toward front)
    {2, 8, 15, false, BOARD_POS_RIGHT_FRONT},  // Board 2: Right front (continues forward)
    {3, 16, 23, false, BOARD_POS_FRONT},       // Board 3: Front (continues right to left)
    {4, 24, 31, true, BOARD_POS_LEFT_FRONT},   // Board 4: Left front (front to back, reversed)
    {5, 32, 39, true, BOARD_POS_LEFT_REAR},    // Board 5: Left rear (front to back, reversed)
...
void neopixel_set_charge_effect(uint8_t charge_percent) {
    ...
    // Add 50 for proper rounding to avoid LED gaps at low charge percentages
    uint8_t num_leds = (charge_percent * 16 + 50) / 100;
